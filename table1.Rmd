...
```{r  results='asis', fig.keep='all', echo=FALSE, results='asis', fig.keep='all', eval=T }
...
```{r fig.align='center',warning=FALSE, cache=T, eval=T}
...
x <- c( 'cd4','hgb','weight','BMI', 'numARVs', 'PillsTakenLast7Days'
       )
raw_df=dataset.cx%>%select(c(y,x)) %>% filter(hgb<20) %>% filter(cd4<1200)
...
for (value in  colnames(cont_univ_df[-ncol(cont_univ_df)]))  { 
  ...
  p1 <- ggplot2.histogram(raw_df%>%drop_na(vl_failure1), xName=value, backgroundColor="white",  bins=10,
   ...
   p2<- ggboxplot(raw_df%>%drop_na(vl_failure1),   alpha=0.2, x = "vl_failure1", y = value, color = "vl_failure1", palette = "jco", add = "jitter", add.params = list(size = size, jitter = jitter,  alpha=0.025))+
   ...
   p3 <- ggplot2.histogram(raw_df%>%drop_na(vl_failure2), xName=value, backgroundColor="white",  bins=10,
                         ...
                         alpha=0.5, addDensity=TRUE,  removePanelGrid=TRUE ,removePanelBorder=TRUE,
                         addMeanLine=TRUE, meanLineSize=1)+ theme_minimal()+ theme(legend.position="top")
   p4<- ggboxplot(raw_df%>%drop_na(vl_failure2), alpha=0.2, x = "vl_failure2", y = value, color = "vl_failure2", palette = "jco", add = "jitter", add.params = list(size = size, jitter = jitter,  alpha=0.025) )+
    stat_compare_means()+ guides(fill=FALSE, color=FALSE)	+ theme_minimal()
...
mergeDesc(
  ...
  "TB sign: weight loss" = getTable1Stats("weightlossTB",outcome,desc_both,dataset.cx,statistics=stats,na.rm=na.rm.var)
   ...
   htmlTable( caption  = "<b> General Clinical Profile Prior to ART Initiation | Viral Load Failure 1 </b>",useViewer=T,ctable = TRUE, align = 'lcccc',
              n.cgroup = c(1,2,1 ), cgroup = c('', 'First Virologic Failure', '') ,tfoot="<sup>&Dagger;</sup>"
              )%>% htmlTable::addHtmlTableStyle(col.rgroup = c("#FFF", "#EEF")) 
...
mergeDesc(
  ...
  "Diagnosis: Progressive Multifocal Leukoencephalopathy" = getTable1Stats("pmldiag",outcome,desc_both,dataset.cx,statistics=stats,na.rm=na.rm.var)
   ...
   htmlTable( caption  = "<b> Clinical Diagnosis Profile Prior to ART Initiation | Viral Load Failure 1 </b>",useViewer=T,ctable = TRUE, align = 'lcccc',
              n.cgroup = c(1,2,1 ), cgroup = c('', 'First Virologic Failure', '') ,tfoot="<sup>&Dagger;</sup>"
              )%>% htmlTable::addHtmlTableStyle(col.rgroup = c("#FFF", "#EEF")) 
...
mergeDesc(
  ...
  "TB treatment" = getTable1Stats("tbtx",outcome,desc_both,dataset.cx,statistics=stats,na.rm=na.rm.var)
   ...
   htmlTable( caption  = "<b> Prophylaxis Profile Prior to ART Initiation | Viral Load Failure 1 </b>",useViewer=T,ctable = TRUE, align = 'lcccc',
              n.cgroup = c(1,2,1 ), cgroup = c('', 'First Virologic Failure', '') ,tfoot="<sup>&Dagger;</sup>"
              )%>% htmlTable::addHtmlTableStyle(col.rgroup = c("#FFF", "#EEF")) 
```
...
mergeDesc(
  ...
  #"ART regimen" = getTable1Stats("regimen",outcome,desc_both,dataset.cx,statistics=stats,na.rm=na.rm.var)
   ...
   htmlTable( caption  = "<b> ARVs Profile Prior to ART Initiation | Viral Load Failure 1 </b>",useViewer=T,ctable = TRUE, align = 'lcccc',
              n.cgroup = c(1,2,1 ), cgroup = c('', 'First Virologic Failure', '') ,tfoot="<sup>&Dagger;</sup>"
              )%>% htmlTable::addHtmlTableStyle(col.rgroup = c("#FFF", "#EEF")) 
